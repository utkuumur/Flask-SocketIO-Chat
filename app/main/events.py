...
def text(message):
    ...
    room = session.get('room')
    enc = base64.b64decode(message['msg'])
    IV = enc[:BLOCK_SIZE]
    hmac = enc[-32:]
    cipher = enc[16:-32]


    if verify_hmac((IV+cipher), hmac, hmac_key):
        emit('message', {
                         'cipher':  base64.b64encode((IV+cipher)).decode(),
                         'mac' : base64.b64encode(hmac).decode(),
